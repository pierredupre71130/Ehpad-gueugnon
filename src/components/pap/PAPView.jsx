...
function Field({ label, value }) {
  ...
  return (
    <div className="pap-field mb-2">
      <div className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-0.5">{label}</div>
      <div className="text-sm text-slate-800 bg-slate-50 border border-slate-100 rounded px-2 py-1.5 whitespace-pre-wrap">{value}</div>
    </div>
  );
}
...
function Section({ title, children }) {
  return (
    <div className="pap-section mb-5">
...
export default function PAPView({ pap, resident, onClose, readOnly, archiveDate }) {
  ...
  const sec = (title, content) => `<div class="section"><div class="section-title">${title}</div>${content}</div>`;
  ...
  const handlePrint = () => {
    ...
    const body = `
      ...
      ${sec("Informations générales", `
        <div class="grid2">
          ${f("Date de naissance", fmtDate(pap.date_naissance))}
          ${f("Service - Chambre", pap.service_chambre)}
          ${f("Date de la réunion", fmtDate(pap.date_reunion))}
          ${f("Date de réévaluation", fmtDate(pap.date_reevaluation))}
        </div>
        ${f("Personnes présentes", pap.presents)}
      `)}

      ${sec("Souhait de la personne concernant son PAP", `
        ${f("Capacité de la personne", pap.capacite)}
        ${f("Souhait de réaliser le projet personnalisé", pap.souhait_projet)}
        ${f("Souhait de participer à la réalisation", pap.souhait_participation)}
        ${f("Souhait de faire participer son entourage", pap.souhait_entourage)}
      `)}

      ${sec("Renseignements généraux", `
        ${f("Données d'identité", pap.donnees_identite)}
        ${f("Souhait concernant sa dénomination", pap.souhait_denomination)}
        ${f("Contexte d'entrée", pap.contexte_entree)}
        ${f("Souhaits de fin de vie", pap.souhaits_fin_vie)}
        ${f("Entourage", pap.entourage)}
        ${f("Droit à l'image", pap.droit_image)}
      `)}

      ${sec("Histoire de vie", `
        ${f("Situation familiale", pap.situation_familiale)}
        ${f("Vie professionnelle", pap.vie_professionnelle)}
        ${f("Épisodes importants", pap.episodes_importants)}
      `)}

      ${sec("Habitudes de vie", `
        <div class="grid2">
          ${f("Boire et manger", pap.besoin_boire_manger)}
          ${f("Éliminer", pap.eliminer)}
          ${f("Se mouvoir", pap.mouvoir_posture)}
          ${f("Dormir et se reposer", pap.dormir_reposer)}
          ${f("Se vêtir", pap.vetir_devtir)}
          ${f("Être propre", pap.propre_teguments)}
          ${f("Éviter les dangers", pap.eviter_dangers)}
          ${f("Communication", pap.communication)}
          ${f("Croyances et valeurs", pap.croyances_valeurs)}
          ${f("Occupation / récréation", pap.occupation_recreation)}
          ${f("Besoin d'apprendre", pap.apprendre)}
          ${f("Ressenti / Adaptation", pap.ressenti_adaptation)}
        </div>
      `)}

      ${sec("Identification des risques", `
        <div class="risks">${risquesHtml}</div>
        ${f("Autres risques", pap.risques_autres)}
      `)}

      ${sec("Remarques particulières", `
        ${f("Accueil des premiers jours", pap.accueil_premiers_jours)}
        ${f("Les soins", pap.soins)}
        ${f("Les repas", pap.repas)}
        ${f("Ambiance générale", pap.ambiance_generale)}
        ${f("Autres remarques", pap.remarques_particulieres)}
      `)}

      ${sec("Objectifs et signature", `
        ${f("Objectifs retenus", pap.objectifs)}
        ${pap.capacite_information ? f("Capacité concernant l'information", CAPACITE_LABELS[pap.capacite_information] || pap.capacite_information) : ""}
        ${f("Date de signature", fmtDate(pap.date_signature))}
        <div class="signature-row">
          <div class="signature-box"><div class="signature-label">Signature du résident</div><div class="signature-area"></div></div>
          <div class="signature-box"><div class="signature-label">Signature du référent</div><div class="signature-area"></div></div>
        </div>
      `)}
    `;
...
<head>
  <meta charset="UTF-8"/>
  <title>PAP — ${resident.last_name} ${resident.first_name}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Arial, sans-serif; font-size: 11px; color: #1e293b; background: white; padding: 10mm; }
    @page { size: A4 portrait; margin: 10mm; }
    h1 { font-size: 15px; font-weight: bold; color: #0f172a; margin-bottom: 2px; }
    .subtitle { font-size: 10px; color: #64748b; margin-bottom: 14px; }
    .section { margin-bottom: 12px; page-break-inside: avoid; }
    .section-title { font-size: 8px; font-weight: bold; color: white; background: #334155; text-transform: uppercase; letter-spacing: 0.08em; padding: 3px 7px; border-radius: 3px; margin-bottom: 5px; }
    .grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 0 14px; }
    .field { margin-bottom: 5px; page-break-inside: avoid; }
    .field-label { font-size: 8px; font-weight: bold; color: #64748b; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 1px; }
    .field-value { font-size: 10px; color: #1e293b; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 3px; padding: 3px 7px; white-space: pre-wrap; }
    .risks { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 5px; }
    .risk-badge { font-size: 8px; background: #fef2f2; color: #b91c1c; border: 1px solid #fecaca; border-radius: 9999px; padding: 2px 7px; }
    .signature-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 14px; }
    .signature-label { font-size: 8px; font-weight: bold; color: #64748b; text-transform: uppercase; margin-bottom: 3px; }
    .signature-area { border: 1px solid #cbd5e1; border-radius: 4px; height: 44px; background: #f8fafc; }
  </style>
</head>
<body>${body}</body>
</html>`);
  ...
  return (
    <>
      ...
      <div id="pap-print-root">
        <div className="pap-modal-overlay fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="pap-modal-box bg-white rounded-xl shadow-2xl w-full max-w-3xl my-4">

            {/* Header */}
            <div className="pap-modal-header flex items-start justify-between px-6 py-4 border-b bg-white rounded-t-xl sticky top-0 z-10">
              <div>
            ...
            {/* Content */}
            <div className="pap-content px-6 py-5">

...
export default function PrintReferentsTable({ residents, onClose }) {
  // Grouper par référent, tri alphabétique
  const grouped = {};
  [...residents]
  ...
  // Référents triés : d'abord ceux qui ont des résidents, "sans référent" en dernier
  ...
  const handlePrint = () => {
    ...
    const rows = sortedRefs.map(ref => {
      ...
      return `
        <tr>
          <td style="font-weight:600; vertical-align:top; padding:8px 12px; background:${isSans ? "#fff7ed" : "#f8fafc"}; border:1px solid #cbd5e1; white-space:nowrap; color:${isSans ? "#9a3412" : "#1e293b"};">
            ${ref}
          </td>
          <td style="padding:8px 12px; border:1px solid #cbd5e1; vertical-align:top;">
            <div style="display:block;">
              ${res.map(r => `
                <div style="font-size:12px; padding:2px 0; display:block;">
                  <strong>${r.last_name}</strong> ${r.first_name || ""} <span style="color:#94a3b8;">— Ch.${r.room}</span>
                </div>
              `).join("")}
            </div>
          </td>
          <td style="padding:8px 12px; border:1px solid #cbd5e1; text-align:center; vertical-align:top; color:#64748b;">
            ${res.length}
          </td>
        </tr>
      `;
...
<head>
  ...
  <style>
    ...
    th { background: #1e293b; color: white; padding: 8px 12px; text-align: left; font-size: 12px; border: 1px solid #1e293b; }
...
<body>
  ...
  <table>
    <thead>
      <tr>
        <th style="width:160px;">Référent</th>
        <th>Résidents attitrés</th>
        <th>Nb</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
              ...
              {sortedRefs.map((ref, i) => {
                ...
                return (
                  <tr key={ref} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    <td className={`px-4 py-3 font-semibold align-top border-b border-slate-100 ${isSans ? "text-orange-700" : "text-slate-800"}`}>
                      {ref}
                    </td>
                    ...
                    <td className="px-4 py-3 border-b border-slate-100 text-center font-bold text-slate-600">
                      {res.length}
                    </td>
                  </tr>
                );
